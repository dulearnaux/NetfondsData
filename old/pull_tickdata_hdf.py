...
def pull_tickdata(toget=['SPX','ETF'], mktdata='all'):
    ...
    log_file_output = open('D:\\Financial Data\\Netfonds\\'+exper +'DailyTickDataPull\\logfile'+datestr+'.txt','w')
    log_file_output2 = open('D:\\Financial Data\\Netfonds\\'+exper +'DailyTickDataPull\\logfile.txt','a')
    ...
    for i in tickers.index:
        ...
        if mktdata=='position' or mktdata=='all':
            data[0].to_csv('D:\\Financial Data\\Netfonds\\'+exper +'DailyTickDataPull\\Positions\\'+folder+name+'.'+date.strftime('%Y%m%d')+'.hdf5')
